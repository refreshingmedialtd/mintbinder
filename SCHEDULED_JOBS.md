...
- `cron-live-international-catalogue.sh` first completes the largest incomplete TCGdex-backed set in the configured languages and verifies its provider total. When no incomplete set remains, it refreshes one bounded 100-card page, choosing the least-recently visited language and advancing that language's durable job-history cursor until wraparound.
