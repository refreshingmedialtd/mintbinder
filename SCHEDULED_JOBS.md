...
TCGCSV publishes one daily price file, not hourly price changes. Card, Japanese, sealed and reviewed-supplement imports use `https://tcgcsv.com/last-updated.txt` as the price observation date and keep import time separate. An unavailable, invalid or future clock fails closed; an old but valid clock stays old so monitoring can detect an upstream stall. Automatic card/sealed rotations skip successfully completed groups already imported for that feed version, reducing duplicate downloads and rows. Explicit group-ID recovery can re-import a version. Partial sealed cursors are never marked completed early. Provider product `modifiedOn` is catalogue metadata, not a price timestamp. See [TCGCSV usage guidelines](https://tcgcsv.com/docs).

Prisma migrations use a direct database connection, while application queries retain the pooled `DATABASE_URL`. `DIRECT_URL` is optional and is checked against the same endpoint, database and schema. For Neon pooled URLs, the CLI automatically derives Neon's documented unpooled hostname without changing runtime configuration or credentials. An advisory-lock timeout must not be bypassed by disabling migration locks or killing unidentified sessions; confirm any abandoned operation and obtain approval before cancelling it. See [Prisma's Neon connection guidance](https://docs.prisma.io/docs/orm/v6/overview/databases/neon).
...
Use `POKEMON_TCG_SET_PRICING_LIMIT="8"` with an hourly schedule for normal card-pricing maintenance. The live helper splits this into one-set HTTP calls by default with `POKEMON_TCG_SET_PRICING_REQUEST_LIMIT="1"`, avoiding 20i gateway timeouts and provider deep-page failures. Each HTTP call processes one provider page and persists the next-page cursor, leaving enough of the gateway window to record a failure and its cooldown. With roughly 173 Pokemon TCG sets in production, eight sets per hour is enough to touch the full set list in about a day. Keep `POKEMON_TCG_PRICING_BATCH_WAIT_MS="1500"` and `POKEMON_TCG_API_RETRY_ATTEMPTS="2"` unless provider stability changes.
...
POKEMON_TCG_SET_PRICING_MAX_PAGES_PER_SET="1"
...
- `cron-live-international-catalogue.sh` first completes the largest incomplete TCGdex-backed set in the configured languages. It imports every card exposed by that exact set endpoint and preserves any larger declared source count separately, avoiding permanent false gaps in partially localized sets. When no incomplete set remains, it refreshes one bounded 100-card page, choosing the least-recently visited language and advancing that language's durable job-history cursor until wraparound.
...
- The existing `cron-live-pricing.sh` command covers Pokemon TCG API set rotation and the capacity-sized TCGCSV English daily rotation. Set discovery is deliberately decoupled into the daily wrapper so it does not add another request to the hourly provider burst. Do not add a second 20i task for English TCGCSV pricing.
...
- CardTrader is the independent European sealed source. Keep its scheduled batch at one set and five products until observed latency is established. With the hourly schedule, `CARDTRADER_SEALED_REFRESH_EVERY_HOURS=4` reserves one UTC-hour slot in four for the least-recently refreshed priced products; the other three slots continue oldest-attempt discovery. Each slot stays inside one expansion and fills spare capacity from the other pool, so this does not increase the request or memory budget. Explicit `CARDTRADER_SEALED_PRODUCT_IDS` imports bypass this automatic split and process only the requested IDs. Marketplace references use eligible English listings and are stored as a separate `cardtrader-sealed` series only when at least three offers form a coherent cluster and the result is not materially divergent from a recent TCGCSV reference. Rejected observations remain auditable in product metadata; historical rejected snapshots are retained under `cardtrader-sealed-quarantined`, which is excluded from every customer-facing valuation and readiness query.
...
- The job monitor treats provider warnings, failed/partial set counts, failed or degraded second sources, and recent failed billing-webhook events as operational degradation. A completed CardTrader discovery batch may legitimately find no safe blueprint or eligible listing, so a recognized API-healthy discovery outcome remains diagnostic rather than making every hourly monitor alert red. A zero-output refresh, an unrecognized result shape, or an API/schema/database failure still degrades the run. Aggregate pricing health also fails if CardTrader coverage falls below 5%, fewer than 75% of its priced products are fresh within 30 days, or its latest evidence is more than 72 hours old. Billing failures include a total count and bounded provider/event/error details in both the report and alert email.
