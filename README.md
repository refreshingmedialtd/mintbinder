...
`npm run qa:authenticated-browser` runs the critical signed-in journey in an installed Chrome-family browser. It covers sign-in, Portfolio catalogue search, card images and price history, Wishlist-to-Collection conversion, an independent direct card add, a priced sealed-product add, owned valuation, default-binder sync, custom-binder creation and manual layout persistence, notification preference persistence, CSV export, insurance PDF progress/download, account JSON export, mobile navigation, and permanent account deletion. First-party HTTP 5xx responses, actionable transport-level request failures, browser errors, console errors, and unexpected dialogs fail the responsible step immediately. Chromium's expected cancellation of obsolete idempotent GET/HEAD requests remains visible separately in the JSON diagnostics; cancelled writes and connection failures still fail the run.
...
Run `npm run qa:square-activation` after a production build for the older local subscription-lifecycle regression pass. It is useful supporting evidence, but its local webhook replay fallback and direct provider records do not prove hosted-checkout payment correlation. `npm run billing:square-smoke` likewise proves only plan and payment-link configuration.
...
Use Square events `subscription.created`, `subscription.updated`, `invoice.payment_made`, `payment.created`, and `payment.updated`. Copy Square's webhook signature key into `SQUARE_WEBHOOK_SIGNATURE_KEY`, and set `SQUARE_WEBHOOK_NOTIFICATION_URL` to the exact URL configured in Square because signature validation depends on that exact value. Hosted subscription checkout can create a different Square customer from the profile Mint Binder prepared, so the payment events carry Mint Binder's signed opaque checkout correlation and attach the resulting customer before Plus is granted. The final configuration uses an independent, high-entropy `SQUARE_CHECKOUT_CORRELATION_SECRET`; `AUTH_SECRET` is only a temporary migration fallback while the public gate is disabled. Existing deployments must follow the two-phase transition below so changing the signing key cannot strand an in-flight checkout. Keep `SQUARE_PAYMENT_CORRELATION_VERIFIED=false` for every sandbox deployment. Square does not provide the same built-in Stripe customer portal flow; set `SQUARE_CUSTOMER_PORTAL_URL` if you have a customer-facing billing management page, or manage early beta subscription changes in Square while we add a fuller in-app cancellation/update flow.

### Hosted-checkout correlation acceptance

The release-gate proof is the sandbox-only, out-of-band runner below. It refuses production Square credentials, refuses an enabled public correlation gate, attests the exact deployed commit, checks that exactly one enabled webhook subscription targets the app, and requires real `payment.created` and `payment.updated` deliveries. It then proves exact order/amount/currency/signature correlation, different-customer attachment, semantic and event-ID idempotence, in-app cancellation, refund, and billing-aware cleanup.

Use this two-phase rollout for the existing deployment:

1. First deploy the correlation-capable code while `SQUARE_CHECKOUT_CORRELATION_SECRET` remains unset or blank in both the deployed 20i `.env` and the operator's ignored local `.env`. Keep `SQUARE_ENVIRONMENT=sandbox` and `SQUARE_PAYMENT_CORRELATION_VERIFIED=false`. This first release deliberately continues signing and validating notes with the existing `AUTH_SECRET`, so a response-loss retry for an already-open attempt remains deterministic.
2. With public checkout still disabled, run `npm run job:live-billing-checkout-retirement`. Resolve every reported ambiguous, provider-error, or completed-pending-reconciliation result and allow or close any still-live legacy link. A Square attempt enters `settling` only when DELETE returns the exact link ID and its exact `cancelled_order_id`, or crash recovery proves the exact stored link is absent and the exact stored order is `CANCELED`; exact-order Payments and tenders must also be empty. A completed order, tender, or searched payment ID is not sufficient payment correlation: the worker must retrieve one unique completed payment and revalidate its signed note, customer, order, amount, currency, and immutable Plus-plan snapshot, otherwise the run remains unhealthy for manual reconciliation. Wait at least 15 minutes and run the job again so it can repeat the canceled-order and empty payment/tender checks before recording `retired_payment_free`; `OPEN`, `DRAFT`, mismatched proof, and any payment evidence fail closed. Repeat until no live or unresolved legacy checkout remains. Then run the read-only `npm run ops:audit-square-correlation-rollout` command against the same database and require its JSON result to contain `"ok": true`; it fails if any Square attempt is retryable, unresolved, uses an unknown or malformed terminal state, or an old `retired`/`failed` row still retains provider state. Do not add or rotate the correlation secret before both checks pass.
3. Only after the rollout audit returns `"ok": true`, generate a new independent secret of at least 32 characters, put the same value in the deployed 20i `.env` and the operator's ignored local `.env`, and redeploy. Do not reuse `AUTH_SECRET` or `JOB_SECRET`, and do not paste the value into logs or screenshots. Leave Square in Sandbox and leave `SQUARE_PAYMENT_CORRELATION_VERIFIED=false` throughout acceptance QA.

After phase 3, set `SQUARE_HOSTED_QA_BASE_URL` to the deployed HTTPS root and `SQUARE_HOSTED_QA_EXPECTED_COMMIT` to the exact 40-character commit reported by `/api/health`. The runner normally reads the deployed webhook address from `SQUARE_WEBHOOK_NOTIFICATION_URL`; if the operator's local value must point somewhere else, set `SQUARE_HOSTED_QA_WEBHOOK_URL` to the exact deployed `/api/billing/webhook/square` URL instead. This override affects only the acceptance runner and must still match the target app root exactly.

Run monthly and yearly as separate disposable accounts:

```sh
npm run qa:square-hosted-correlation -- --plan=monthly
npm run qa:square-hosted-correlation -- --plan=yearly
```

For each run, leave the command open and complete only the one URL it prints, using the printed run-scoped buyer details and Square's sandbox Visa. The command never prints the disposable Mint Binder password. It stores resumable recovery state under the ignored `.local-square-qa` directory and deletes that file after verified cleanup. If a recoverable run stops, use the exact `--resume=<run-id>` command it prints. Use `--abort=<run-id>` only before payment; the runner refuses raw cleanup after any payment or live subscription is observed.

A successful JSON report for both plans is the evidence needed to approve the correlation design. Do not change `SQUARE_PAYMENT_CORRELATION_VERIFIED` while the deployment uses sandbox credentials: doing so would expose test-card Plus checkout to every verified user. The legacy-attempt drain and `"ok": true` rollout audit belong between the first fallback-compatible deployment and the second deployment that introduces the dedicated secret; it is not safe to postpone them until the later false-to-true flag transition. Once verified mode is enabled, only the dedicated secret is accepted. At paid launch, switch to reviewed production Square credentials and set the flag to `true` in the same controlled release. Future correlation-secret rotation must likewise disable checkout, drain every live or unresolved attempt, require the rollout audit to pass, and only then change the key.
...
Use `npm run report:catalogue-gaps` or the Operations export button to check local catalogue health, set-level count gaps, duplicate Pokemon TCG provider IDs, image coverage, variant metadata coverage, pricing-source mix, sealed product-type gaps, and recommended next actions. Operations can load a duplicate provider review showing each duplicate group, affected rows, and attached collection/wishlist/price usage before any manual merge decision. After review, use Prepare to fill the primary and duplicate card IDs, dry-run the plan, then execute it to move collection items, wishlist items, and price snapshots onto the primary card before deleting the duplicate card row. Pokemon TCG card image URLs can be repaired from provider IDs in Operations or from the command line with `npm run job:repair-card-images`; set `CARD_IMAGE_REPAIR_LIMIT` and `CARD_IMAGE_REPAIR_DRY_RUN=true` for a smaller or preview-only run. TCGCSV sealed product image URLs can be repaired with `npm run job:repair-sealed-images`; set `SEALED_IMAGE_REPAIR_LIMIT`, `SEALED_IMAGE_REPAIR_DRY_RUN=true`, and `SEALED_IMAGE_REPAIR_WAIT_MS` to tune the run. A provider URL confirmed unavailable can be audited with `npm run ops:quarantine-broken-sealed-images -- --ids=<sealed-product-id>` and applied only after review with the same command plus `--confirm`. This operation is never scheduled: it re-probes before applying, requires a working same-host control image, caps mutations independently, and stores evidence so imports and repairs skip only the exact quarantined URL while accepting a future replacement. Unknown or malformed options are rejected; a confirmed run without IDs is refused unless the operator deliberately supplies `--broad`. Broad scans do not apply HTTP 400/403 results because provider WAF or hotlink protection can mimic those responses. Pokemon TCG variant choices can be repaired with `npm run job:repair-variant-metadata`; set `VARIANT_METADATA_REPAIR_LIMIT`, `VARIANT_METADATA_REPAIR_DRY_RUN=true`, and `VARIANT_METADATA_REPAIR_WAIT_MS` to tune the API backfill. For command-line pricing refreshes, set `JOB_SECRET`, `POKEMON_TCG_PRICING_PAGE`, `POKEMON_TCG_PRICING_PAGE_SIZE`, `POKEMON_TCG_PRICING_MAX_PAGES`, and optionally `POKEMON_TCG_PRICING_QUERY`, then run `npm run job:pricing-batch`. For several known sparse sets, set `POKEMON_TCG_PRICING_QUERIES` to comma-separated queries such as `set.id:sm1,set.id:sm5` and run `npm run job:pricing-targets`; this starts one local job server and processes each one-page target sequentially. Automatic exchange rates enable Pokemon TCG API Cardmarket fallback prices by default; set `POKEMON_TCG_PRICE_ONLY_UNPRICED=true` when enriching sparse segments without duplicating existing snapshots.

For TCGCSV card-pricing enrichment, run `npm run job:tcgcsv-card-pricing`. The importer reads TCGCSV's cached TCGplayer Pokemon groups/products/prices, matches groups to local card sets, matches card products by set/name/number, and writes card price snapshots with source `tcgcsv-card`. When a matched product has multiple price subtypes, each usable subtype is stored as its own variant snapshot. Set `TCGCSV_CARD_GROUP_IDS` to a comma-separated list of TCGplayer group IDs for a targeted run, `TCGCSV_USD_TO_GBP_RATE` or `POKEMON_TCG_USD_TO_GBP_RATE` for this standalone importer, and `TCGCSV_CARD_PRICE_ONLY_UNPRICED=true` only for a deliberate missing-price backfill. The hourly production wrapper uses `priceOnlyUnpriced=false` and processes two oldest-priced groups serially so the full mapped rotation stays comfortably inside the seven-day freshness window. Scheduled imports only read historical identities to keep new labels stable and never relabel old snapshots. Audit legacy identity collisions with `npm run ops:repair-tcgcsv-price-identities`; it is read-only by default and refuses `--confirm` while an affected label is referenced by an active or archived collection item or wishlist item. Migrate those user references first; `--allow-ambiguous-user-variants` is an explicit last-resort override after manual review. These prices remain US market references rather than UK market prices.

`npm run job:live-reviewed-card-catalogue` maintains provider-backed catalogue gaps and exact reviewed finishes omitted by the primary English catalogue. Before pricing MEP promos, it synchronises the complete live English MEP set from TCGdex and verifies that the local card count reaches the provider-declared total; this automatically includes later promo numbers such as Fennekin 080 instead of relying on one-card patches. TCGdex's immutable TCGplayer product IDs preserve regular, Pokémon Center, set-logo, staff, and patterned-foil identities when TCGCSV prices the same products. The narrower immutable-product-ID allowlist still covers the three English-only Trading Card Game Classic tool cards, Japanese Emerald Break Rayquaza EX 061/078, and the reviewed White Flare/Stellar Crown stamped products. Intentionally partial supplements publish their supported local card total while retaining the upstream set count as `sourceCardCount`, so catalogue-gap monitoring does not pretend unsupported source rows are missing local records. Each allowlisted product name and collector number must still match its approved identity or the job fails closed. The daily wrapper runs every stage as a separate tracked request and does not report complete when a full provider set count is unmet. Set `TCGCSV_REVIEWED_CATALOGUE_GROUP_IDS` only for a targeted recovery run; leave it empty for normal daily maintenance.

For international catalogue backfills, use Operations or run `npm run job:live-international-catalogue-backfill` against the deployed app. The scheduled international refresh first selects the largest incomplete TCGdex-backed set in its configured languages and synchronises that complete set directly; only when no deficit remains does it advance the ordinary whole-catalogue page cursor. Some localized TCGdex sets declare the original set size but publish fewer localized card records. In that case Mint Binder imports every card the set endpoint exposes, uses that available count for local completion, and retains the larger declaration as `sourceCardCount` for audit rather than inventing missing cards. By default, manual backfill reruns the source-backed Simplified Chinese and Korean TCGdex imports in small chunks; set `TCGDEX_BACKFILL_LANGUAGES=ja,zh-tw,zh-cn,ko` when you deliberately want to refresh every supported international language. A newly enabled Japanese pricing lane must first complete the Japanese catalogue rather than loosen its matcher: run the same command with `TCGDEX_BACKFILL_LANGUAGES=ja`, then confirm that `catalogueIncompleteGroups` falls to zero (or that each reported local set has its expected card count) before judging Japanese price-match yield.
...
For production scheduled jobs, see [SCHEDULED_JOBS.md](SCHEDULED_JOBS.md). The live helpers call the deployed app instead of starting a temporary local server: `npm run job:live-health`, `npm run job:live-password-reset-delivery`, `npm run job:live-billing-checkout-retirement`, `npm run job:live-pricing`, `npm run job:live-english-card-pricing`, `npm run job:live-japan-card-pricing`, `npm run job:live-sealed-pricing`, and `npm run job:live-price-alerts`. Run password-reset delivery every minute so the public request endpoint only performs a durable, enumeration-safe enqueue; stale or uncertain claims are surfaced by monitoring and never automatically resent. Run billing checkout retirement every ten minutes so expired hosted links are checked against provider truth and closed without waiting for another user request. Hourly pricing is separate from `scripts/cron-live-daily.sh`, which handles slow set discovery and a forced-safe price-alert dry run. When `CARDTRADER_API_TOKEN` is configured, the sealed-pricing route also rotates through a small CardTrader batch. One of every four hourly slots refreshes already-priced mappings while the other three continue conservative discovery, without increasing the one-set/five-product request budget; direct targeted imports bypass that split. It resolves blueprints conservatively within a matched expansion using a reviewed manual alias, exact TCGplayer ID, exact UPC/EAN, or exact normalized name plus compatible product type; ambiguous candidates are emitted for review rather than guessed. Recognized API-healthy discovery misses remain visible as explicit outcomes without becoming operational failures; zero-output refreshes and malformed provider responses remain actionable, while aggregate coverage, freshness, and last-evidence thresholds alert on longer-term stalls. Eligible English asks are reduced to a conservative low-listing median, then must have at least three coherent offers and stay within the configured material-divergence threshold of a recent TCGCSV reference. Sparse, dispersed, or reference-divergent observations are retained in sealed-product audit metadata with an explicit quarantine status and never written as trusted `cardtrader-sealed` price snapshots. Obtain the bearer token from the settings page of the CardTrader account used by the service; leave `CARDTRADER_SEALED_ENABLED` empty to enable the lane automatically when the token exists.

After first deploying the CardTrader guardrails, audit historical snapshots with `npm run ops:quarantine-cardtrader-outliers`. This is read-only by default and reports the exact rows that would be re-sourced. Review the sample, then run `npm run ops:quarantine-cardtrader-outliers -- --confirm` to retain rejected rows and their evidence under `cardtrader-sealed-quarantined`. That source is permanently excluded from customer valuations, catalogue ordering, price history, reports, and readiness coverage. The repair is idempotent because confirmed rows leave the active `cardtrader-sealed` scope.
