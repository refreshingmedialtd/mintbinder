...
`npm run job:live-reviewed-card-catalogue` maintains the small, immutable-product-ID allowlist for cards or parallel finishes omitted by the normal catalogue providers. It currently covers MEP Tyrunt 070 (regular and Pokémon Center stamp), the three English-only Trading Card Game Classic tool cards, Japanese Emerald Break Rayquaza EX 061/078, and the reviewed White Flare/Stellar Crown stamped products. Each product name and collector number must still match its approved identity or the job fails closed. The daily wrapper runs every reviewed group as a separate tracked request, creates exact variant-labelled price history, and then lets the ordinary English/Japanese TCGCSV rotations maintain the newly mapped non-miscellaneous groups. Set `TCGCSV_REVIEWED_CATALOGUE_GROUP_IDS` only for a targeted recovery run; leave it empty for normal daily maintenance.
